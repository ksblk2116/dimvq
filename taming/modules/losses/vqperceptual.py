...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start, codebook_weight=0., pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 commit_weight = 0.25, codebook_enlarge_ratio=0, codebook_enlarge_steps=2000,
    ...
    def forward(self, disentangle_loss, loss_break, inputs, reconstructions, optimizer_idx,
        ...
        if optimizer_idx == 0:
            ...
            # Scale disentangle loss (from SimVQ groups orthogonality)
            scaled_disentangle_loss = self.disentangle_weight * disentangle_loss
            
            # Legacy codebook loss scaling (for entropy loss if used)
            scale_codebook_loss = self.codebook_weight * disentangle_loss  # kept for backward compatibility
            ...
            loss = nll_loss + g_loss + scaled_disentangle_loss + loss_break.commitment * self.commit_weight
            if disc_factor == 0:
                log = {"{}/total_loss".format(split): loss.clone().detach(),
                       ...
                       "{}/disentangle_loss".format(split): disentangle_loss.detach() if torch.is_tensor(disentangle_loss) else torch.tensor(disentangle_loss),
                ...
                if self.training:
                    log = {"{}/total_loss".format(split): loss.clone().detach(),
                           ...
                           "{}/commit_loss".format(split): loss_break.commitment.detach(),
                           "{}/disentangle_loss".format(split): disentangle_loss.detach() if torch.is_tensor(disentangle_loss) else torch.tensor(disentangle_loss),
                           "{}/scaled_disentangle_loss".format(split): scaled_disentangle_loss.detach() if torch.is_tensor(scaled_disentangle_loss) else torch.tensor(scaled_disentangle_loss),
                           "{}/nll_loss".format(split): nll_loss.detach(),
                           "{}/reconstruct_loss".format(split): rec_loss.detach().mean(),
                           "{}/perceptual_loss".format(split): p_loss.detach().mean(),
